...
import { featuredFruits, categories } from '../../Assets/Data/data';
...
const HomeScreen = () => {
    ...
    return (
      <SafeAreaView className="flex-1 bg-orange-50">
          ...
          <View className="mt-4">
            <Text  
            className="text-2xl tracking-widest font-medium ml-5 bg-orange-500">Seasonal</Text>
            <Text   className="text-3xl font-semibold ml-5 bg-orange-500">Fruits and Vegetables</Text>
            <ScrollView className="mt-8 px-5" horizontal showsHorizontalScrollIndicator={false}>
              {
                categories.map((category, index)=>{
                  let isActive = category == activeCategory;
                  let textClass = `text-xl bg-orange-500   ${isActive? ' font-bold': ''}`;
